export default function TarjetaMetrica({ titulo, valor, detalle, progreso = 0, icono: Icono, tono = "orange" }) {
  const tonos = {
    orange: {
      icono: "bg-orange-50 text-orange-600",
      barra: "bg-orange-500",
  ...
  return (
    <article className="group rounded-2xl border border-stone-200 bg-white p-5 shadow-card transition-all duration-200 hover:-translate-y-0.5 hover:shadow-card-hover">
      <div className="flex items-start justify-between gap-4">
        <div className="min-w-0">
          <p className="truncate text-xs font-semibold uppercase tracking-wide text-stone-400">{titulo}</p>
        ...
        <div className={`grid h-11 w-11 shrink-0 place-items-center rounded-xl transition-transform duration-200 group-hover:scale-105 ${estilos.icono}`}>
          <Icono size={22} />
        </div>
      </div>
      ...
      <p className={`mt-1.5 text-right text-[10px] font-semibold ${estilos.accent}`}>{Math.round(progreso)}%</p>
    </article>
  );
}
