export default function Selector({ label, error, children, className = "", ...props }) {
  return (
    <label className={`block ${className}`}>
      ...
      <select className="w-full min-w-0 rounded-2xl border border-stone-200 bg-white px-4 py-3 text-stone-900 outline-none transition focus:border-orange-300 focus:ring-4 focus:ring-orange-100" {...props}>
        {children}
      </select>
